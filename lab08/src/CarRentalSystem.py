...
from src.exception import BookingError
...
class CarRentalSystem:
    ...
    def make_booking(self, customer, period, car, location):
        try:
            if len(location.pickup) == 0:
                raise BookingError('location', 'Please enter a valid pick up location')
            if len(location.dropoff) == 0:
                raise BookingError('location', 'Please enter a valid drop of location')
            if period.days < 1:
                raise BookingError('period', 'Please enter a valid start and end time')
